...
import { Loader2, ArrowLeft, CheckCircle, Mail, Smartphone, FileText, Lock, Award, Gavel, ScrollText } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';

// --- CONFIGURATION COULEUR ET TEXTES (Verts moins foncés) ---
const PRIMARY_COLOR = 'bg-green-600';      // Vert plus moderne pour la connexion
const SECONDARY_COLOR = 'bg-lime-500';     // Vert clair pour les boutons
const TERTIARY_COLOR = 'bg-yellow-400';    // Jaune pour Inscription Étape 1
const QUATERNARY_COLOR = 'bg-cyan-500';    // Cyan professionnel pour Inscription Étape 2
const TEXT_COLOR_LIGHT = 'text-white';
const TEXT_COLOR_DARK = 'text-gray-900';

// Icônes Thématiques pour les documents d'état
const SignInIllustration = Gavel;       // Marteau de Juge pour l'autorité/professionnel
const Step1Illustration = FileText;    // Document pour l'enregistrement
const Step2Illustration = Lock;        // Sécurité
const Step3Illustration = Award;       // Succès

// NOTE: Pour simuler l'animation de flottement (animate-float), nous utiliserons 'animate-bounce' 
// ou créerons une classe personnalisée dans le CSS/global (non possible ici). 
// J'utiliserai un mouvement 'animate-pulse' combiné à 'duration-500' pour la fluidité.

// --- Composant d'Icônes Flottantes ---
const FloatingIcons = ({ theme }: { theme: 'light' | 'dark' }) => {
    const iconColor = theme === 'light' ? 'text-white/20' : 'text-gray-900/10';

    return (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {/* Icône Marteau de Juge (Gavel) - Mouvement lent et flottant */}
            <Gavel 
                className={`absolute top-[10%] left-[20%] h-16 w-16 ${iconColor} opacity-50 transform rotate-12 animate-pulse transition-all duration-300`} 
                style={{ animationDelay: '0s', animationDuration: '4s' }} 
            />
            {/* Icône Document (FileText) */}
            <FileText 
                className={`absolute bottom-[15%] right-[15%] h-12 w-12 ${iconColor} opacity-50 transform -rotate-6 animate-pulse transition-all duration-300`} 
                style={{ animationDelay: '1s', animationDuration: '5s' }} 
            />
            {/* Icône Scroll/Document d'État */}
            <ScrollText 
                className={`absolute top-[50%] right-[5%] h-10 w-10 ${iconColor} opacity-50 transform rotate-45 animate-pulse transition-all duration-300`} 
...
const ProgressIndicator = ({ currentStep, totalSteps = 3 }: { currentStep: number, totalSteps?: number }) => (
    <div className="flex justify-center space-x-2 my-6">
        {Array.from({ length: totalSteps }).map((_, index) => (
            <div 
                key={index} 
                className={`h-2 w-2 rounded-full transition-all duration-500 ${ // Transition plus fluide
                    index + 1 === currentStep 
...
const SignUpWizard = ({ setMode }: { setMode: (mode: 'signin' | 'signup') => void }) => {
    ...
    const renderStep1 = () => (
        <form onSubmit={handleStep1Submit} className="flex flex-col h-full p-8 transition-colors duration-500 font-sans-modern">
            <FloatingIcons theme='dark' /> {/* Icônes flottantes sombres sur fond clair */}
            ...
            <div className={`flex-1 flex flex-col justify-center items-center text-center ${currentTextColor} relative z-10`}>
    ...
    const renderStep2 = () => (
        <form onSubmit={handleFinalSignUp} className="flex flex-col h-full p-8 transition-colors duration-500 font-sans-modern">
            <FloatingIcons theme='light' /> {/* Icônes flottantes claires sur fond coloré */}
            <div className="flex justify-end relative z-10">
    ...
    const renderStep3 = () => (
        <div className="flex flex-col h-full p-8 text-white transition-colors duration-500 font-sans-modern">
             <FloatingIcons theme='light' />
            ...
            <div className="mb-4 flex flex-col items-center">
                <Button onClick={() => navigate('/dashboard')} className={`w-full max-w-sm h-14 text-xl font-bold ${PRIMARY_COLOR} hover:bg-green-700 transition-colors shadow-2xl rounded-full hover:scale-[1.02] active:scale-[0.98]`}>
...
const SignInForm = ({ setMode }: { setMode: (mode: 'signin' | 'signup') => void }) => {
    ...
    return (
        <div className={`min-h-screen ${PRIMARY_COLOR} transition-colors duration-500 flex flex-col overflow-hidden font-sans-modern`}>
            <FloatingIcons theme='light' /> {/* Icônes flottantes claires sur fond coloré */}
            <div className="flex-1 max-w-md w-full mx-auto p-8 relative">
                ...
                <div className={`flex-1 flex flex-col justify-center items-center text-center ${TEXT_COLOR_LIGHT} pt-12 relative z-10`}>
                    <div className="p-6 rounded-full bg-white/40 mb-8 animate-pulse">
                ...
                <div className="absolute bottom-4 left-0 right-0 p-4 text-center relative z-10">
