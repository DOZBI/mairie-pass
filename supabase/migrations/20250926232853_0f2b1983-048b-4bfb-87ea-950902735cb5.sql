...
-- Create storage bucket for identity card photos
INSERT INTO storage.buckets (id, name, public) 
VALUES ('identity-cards', 'identity-cards', false);

-- Create storage policies for identity card uploads
CREATE POLICY "Users can view their own identity cards" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'identity-cards' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can upload their own identity cards" 
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'identity-cards' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can update their own identity cards" 
ON storage.objects 
FOR UPDATE 
USING (bucket_id = 'identity-cards' AND auth.uid()::text = (storage.foldername(name))[1]);
